...
export default {
    ...
    components: {},
    mounted() {
        this.geoNowMapList = this.convertData(this.ChinaData);
        this.drawData('AirChina');
    },
    methods: {
        makeMapData(rawData) {
            var mapData = [];
            for (var i = 0; i < rawData.length; i++) {
                var geoCoord = this.geoWorldMap[rawData[i][0]];
                if (geoCoord) {
                    mapData.push({
                        name: rawData[i][0],
        ...
        convertData(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                var geoCoord = this.geoChinaMap[data[i].name];
                if (geoCoord) {
                    res.push({
                        name: data[i].name,
        ...
        drawData(id) {
            ...
            this.chart.setOption({
                title: {
                    show: false,
                    text: 'Air quality in major cities in China',
                    subtext: '全国主要城市空气质量',
                ...
                series: [{
                    ...
                    // type: 'effectScatter',
                    coordinateSystem: 'geo',
                    data: this.makeMapData(this.WorldData),
                    symbolSize: 11,
                    label: {
                        normal: {
                            formatter: '{b}',
